...
static void update_bank_1(CPU_t *cpu) {
	...
	if (ram) {
		...
		change_page(cpu->mem_c, 1, (unsigned char)page, ram);
	} else {
		page = (cpu->mem_c->port06 & 0x7F) | (cpu->mem_c->port0E << 7);
		change_page(cpu->mem_c, 1, (unsigned char)(page & (cpu->mem_c->flash_pages - 1)), ram);
	}
}
...
static void update_bank_2(CPU_t *cpu) {
	...
	if (ram) {
		...
		change_page(cpu->mem_c, 2, (unsigned char)page, ram);
	} else {
		page = (cpu->mem_c->port07 & 0x7F) | (cpu->mem_c->port0F << 7);
		change_page(cpu->mem_c, 2, (unsigned char)(page & (cpu->mem_c->flash_pages - 1)), ram);
	}
}
