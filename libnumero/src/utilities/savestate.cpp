...
BOOL LoadSlot(SAVESTATE_t *save, const LPCALC lpCalc) {
	...
	/*catch (std::exception& e) {
		printf(_T("Exception loading save state: %s"), e.what());
		return FALSE;
	}*/
}
...
void WriteSave(const TCHAR *fn, SAVESTATE_t* save, int compress) {
	...
	int smurf = sizeof(DETECT_STR);

	filestream_write(ofile, DETECT_STR, sizeof(DETECT_STR)-1);

	//fputs(DETECT_STR, ofile);

	tempInt[0] = SAVE_HEADERSIZE; filestream_write(ofile, tempInt, sizeof(int));
	tempInt[0] = save->version_major; filestream_write(ofile, tempInt, sizeof(int));
	tempInt[0] = save->version_minor; filestream_write(ofile, tempInt, sizeof(int));
	tempInt[0] = save->version_build; filestream_write(ofile, tempInt, sizeof(int));
	tempInt[0] = save->model; filestream_write(ofile, tempInt, sizeof(int));
	tempInt[0] = save->chunk_count; filestream_write(ofile, tempInt, sizeof(int));
	...
	filestream_write(ofile, save->author, 32);
	filestream_write(ofile, save->comment, 64);

	//fwrite(save->author, 1,32, ofile);
	//fwrite(save->comment, 1, 64, ofile);
	
	for(i = 0; i < save->chunk_count; i++) {
		filestream_putc(ofile, save->chunks[i]->tag[0]);
		filestream_putc(ofile, save->chunks[i]->tag[1]);
		filestream_putc(ofile, save->chunks[i]->tag[2]);
		filestream_putc(ofile, save->chunks[i]->tag[3]);
		filestream_write(ofile, &(save->chunks[i]->size), sizeof(int));
		filestream_write(ofile, save->chunks[i]->data, save->chunks[i]->size);
...
SAVESTATE_t* ReadSave(RFILE *ifile) {
	int i;
	...
	filestream_read(ifile,string, 8 );
	string[8] = 0;
	if (strncmp(DETECT_CMP_STR, string, 8) == 0) {
		i = filestream_getc(ifile);
		tmpfile_s(&tmpFile);
		if (!tmpFile) {
			return NULL;
		}
	...
	if (!save) {
		...
		if (compressed == TRUE) filestream_close(ifile);
		return NULL;
	}

	filestream_read(ifile, &chunk_offset,4);

	filestream_read(ifile, &(save->version_major), 4);
	filestream_read(ifile, &(save->version_minor), 4);
	filestream_read(ifile, &(save->version_build), 4);
	...
	filestream_read(ifile, &(save->model), 4);
	//save->model = (CalcModel) fgeti(ifile);

	filestream_read(ifile, &(chunk_count), 4);
	//chunk_count = fgeti(ifile);

	filestream_read(ifile, &(save->author), MAX_SAVESTATE_AUTHOR_LENGTH);
	filestream_read(ifile, &(save->comment), MAX_SAVESTATE_COMMENT_LENGTH);

	//fread(save->author, 1, MAX_SAVESTATE_AUTHOR_LENGTH, ifile);
	//fread(save->comment, 1 , MAX_SAVESTATE_COMMENT_LENGTH, ifile);

	filestream_seek(ifile, chunk_offset + 8 + 4, SEEK_SET);
	...
	for (i = 0; i < chunk_count; i++) {
		string[0] = filestream_getc(ifile);
		string[1] = filestream_getc(ifile);
		string[2] = filestream_getc(ifile);
		string[3] = filestream_getc(ifile);
		...
		string[4] = 0;
		if (filestream_eof(ifile)) {
			FreeSave(save);
			return NULL;
		}
		chunk = NewChunk(save, string);
		filestream_read(ifile, &(chunk->size), 4);
		...
		filestream_read(ifile, chunk->data, chunk->size);
	}
