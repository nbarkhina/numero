...
BOOL check_flashpage_empty(unsigned char(*dest)[PAGE_SIZE], unsigned int page, unsigned int num_pages) {
	unsigned char *space = &dest[page][PAGE_SIZE - 1];
	unsigned int i;
...
*/
void fix_certificate(CPU_t *cpu, unsigned int page) {

	unsigned char(*dest)[PAGE_SIZE] = (unsigned char(*)[PAGE_SIZE]) cpu->mem_c->flash;
	upages_t upages;
...
LINK_ERR forceload_os(CPU_t *cpu, TIFILE_t *tifile) {
	...
	for (i = 0; i < ARRAYSIZE(tifile->flash->data); i++) {
		...
		if (i >= (unsigned int)cpu->mem_c->flash_pages) {
			break;
		}
...
static LINK_ERR replace_app(CPU_t *cpu, TIFILE_t *tifile, upages_t upages, unsigned char(*dest)[PAGE_SIZE], int page) {
	unsigned short pageDiff = (unsigned short)(tifile->flash->pages - get_page_size(dest[page]));
	unsigned int currentPage = page - tifile->flash->pages;
	unsigned int end_page = pageDiff > 0 ? currentPage : currentPage + pageDiff;
	...
	if (end_page != currentPage) {
		if (pageDiff > 0) {
			...
			if (cpu->pio.model == TI_83P) {
				...
				for (unsigned int i = end_page - 7; i <= end_page + pageDiff - 8; i++) {
			...
			for (unsigned int i = tifile->flash->pages; i < tifile->flash->pages - pageDiff; i++, currentPage--) {
